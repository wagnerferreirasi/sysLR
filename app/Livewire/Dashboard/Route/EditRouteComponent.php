...
class EditRouteComponent extends Component
{
    ...
    {
        $this->dispatch('alert', ['type' => 'info',  'message' => 'Atualizando rota, aguarde ser redirecionado...']);
        ...
        try {
            ...
            $this->dispatch('alert', ['type' => 'success',  'message' => 'Rota atualizada com sucesso!']);
            return redirect()->route('dashboard.routes');
        } catch (\Exception $e) {
            $this->dispatch('alert', ['type' => 'error',  'message' => 'Erro ao atualizar rota!' . $e->getMessage()]);
        }
    }
}
